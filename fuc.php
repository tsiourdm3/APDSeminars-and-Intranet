...
function login()
{
    ...
    if (empty($username))
    {
        array_push($errors, "<p style='color:red'>Παρακαλώ εισάγεται username</p>");
    }
    if (empty($password))
    {
        array_push($errors, "<p style='color:red'>Απαιτείται κωδικός πρόσβασης</p>");
    }
    ...
    if (count($errors) == 0)
    {
        ...
        { // user found
            ...
            mysqli_query($db, "INSERT INTO userlog(userID,username,userIp,loginTime,action) VALUES ('" . $_SESSION['id'] . "','" . $_SESSION['username'] . "','$uip',now(),'$action')");
        ...
        {
            array_push($errors, '<p style="color:red">Λάθος κωδικός ή username. Προσπαθήστε ξανά!</p>');
        }
        // if user is admin, redirect to admin area
